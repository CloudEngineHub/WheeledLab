...
from mppi.utils import resolve_obj
...
class MPPI(torch.nn.Module):
    def __init__(self,
        ...
        self._rollouts_shape = (self.num_envs, self.sampling.K, self.sampling.T + 1, self.dynamics.state_dim)
    ...
    def step(self, x0, map, use_prev_opt:bool=True) -> torch.Tensor:
        ...
        for i in range(self.cfg.opt_iters):
            ...
            actions = self.optimize(x0, iter=i)
    ...
    def optimize(self, x0, iter=None) -> torch.Tensor:
        """
    ...
    def create_vis(self):
        ...
        with torch.no_grad():
            ...
            elevation_map = self.bevmap.map.cpu().numpy() if self.bevmap else None
